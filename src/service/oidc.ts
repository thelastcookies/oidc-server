 ...
 *
 * @param data.client_id - 客户端标识（如 "my-app"）
 * @param data.client_secret - 客户端密钥（bcrypt 哈希后存储）
 * @param data.redirect_uris - 授权回调地址白名单
 * @param data.grant_types - 允许的授权类型（默认 authorization_code + refresh_token）
 * @param data.response_types - 允许的响应类型（默认 code，即授权码模式）
 * @param data.token_endpoint_auth_method - 令牌端点认证方式（默认 client_secret_post）
 */
...
export const updateClient = async (
  ...
  const existingData = JSON.parse(client.data);
  const updatedData = { ...existingData };

  if (data.client_name) updatedData.client_name = data.client_name;
  if (data.redirect_uris) updatedData.redirect_uris = data.redirect_uris;
  if (data.post_logout_redirect_uris) updatedData.post_logout_redirect_uris = data.post_logout_redirect_uris;
  if (data.grant_types) updatedData.grant_types = data.grant_types;
  if (data.response_types) updatedData.response_types = data.response_types;
